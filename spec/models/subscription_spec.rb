...
describe Subscription do
  ...
  it "should be created with a renewal date based on the subscription plan renewal period" do
    ...
    s = Subscription.create(:plan => @basic)
    s.next_renewal_at.localtime.at_midnight.should == Time.now.advance(:months => 3).at_midnight
  end
...
def paypal_profile_options(opts = {})
  { :description => Saas::Config.app_name, :start_date => @start_date, :frequency => 1, :amount => @sub.amount_in_pennies }.merge(opts)
end
